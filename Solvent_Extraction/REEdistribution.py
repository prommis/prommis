...
@declare_process_block_class("REESolExParameters")
class REESolExParameterData(PhysicalParameterBlock):
    def build(self):
        super().build()
        ...
        self._state_block_class = REESolExStateBlock
...
class _REESolExStateBlock(StateBlock):
    def fix_initialization_states(self):
        fix_state_vars(self)

@declare_process_block_class("REESolExStateBlock", block_class=_REESolExStateBlock)
class REESolExStateBlockData(StateBlockData):
    def build(self):
        super().build()
    ...
    def get_material_flow_terms(self, j):
        if j in self.params.dissolved_elements:  
            ...
            return self.volumetric_flow * (1840 * units.g/units.L) 
        else:
            raise BurntToast()
