...
**False** - exclude pressure change terms.}"""))
    ...
    def _make_params(self):
        ...
        self.ree_list_all = ["Sc", "Y", "La", "Ce", "Pr", "Nd", "Pm", "Sm", "Eu", "Gd", \
        ...
        self.am_comp_organic_ele = Param(self.organic_ele_list, mutable=True, units=pyunits.kg/pyunits.mol)
        ...
        # atomic mass of REE
        self.am_ree_list_all = Param(self.ree_list_all, mutable=True, units=pyunits.kg/pyunits.mol)
        ...
        self.mw_CO2 = Param(initialize=self.am_C + self.am_O*2, units=pyunits.kg/pyunits.mol)
        self.mw_SO2 = Param(initialize=self.am_S + self.am_O*2, units=pyunits.kg/pyunits.mol)
        self.mw_Un2O3 = Param(initialize=self.am_Un*2 + self.am_O*3, units=pyunits.kg/pyunits.mol)
        self.mw_CaO = Param(initialize=self.am_Ca + self.am_O, units=pyunits.kg/pyunits.mol)
        self.mw_SiO2 = Param(initialize=self.am_Si + self.am_O*2, units=pyunits.kg/pyunits.mol)
        self.mw_Al2O3 = Param(initialize=self.am_Al*2 + self.am_O*3, units=pyunits.kg/pyunits.mol)
        self.mw_Fe2O3 = Param(initialize=self.am_Fe*2 + self.am_O*3, units=pyunits.kg/pyunits.mol)
        self.mw_CaCO3 = Param(initialize=self.am_Ca + self.am_C + self.am_O*3, units=pyunits.kg/pyunits.mol)
        self.mw_Kaolinite = Param(initialize=self.mw_Al2O3 + self.mw_SiO2*2 + self.mw_H2O*2, units=pyunits.kg/pyunits.mol)
        self.mw_Pyrite = Param(initialize=self.am_Fe + self.am_S*2, units=pyunits.kg/pyunits.mol)

        # molecular weights of impurity mineral species in solid feed
        self.mw_comp_impurity = Param(self.impurity_list, mutable=True, units=pyunits.kg/pyunits.mol)
        ...
        self.mw_comp_product = Param(self.product_list, mutable=True, units=pyunits.kg/pyunits.mol)
        ...
        # Cp = Cp0 + Cp1*T
        self.cp0_Un2O3 = Param(initialize=28.039, units=pyunits.J/pyunits.mol/pyunits.K)
        self.cp0_CaO = Param(initialize=47.563, units=pyunits.J/pyunits.mol/pyunits.K)
        self.cp0_SiO2 = Param(initialize=31.414, units=pyunits.J/pyunits.mol/pyunits.K)
        self.cp0_Al2O3 = Param(initialize=28.039, units=pyunits.J/pyunits.mol/pyunits.K)
        self.cp0_Fe2O3 = Param(initialize=80.623, units=pyunits.J/pyunits.mol/pyunits.K)
        self.cp0_CaCO3 = Param(initialize=76.009, units=pyunits.J/pyunits.mol/pyunits.K)
        self.cp0_Pyrite = Param(initialize=59.09, units=pyunits.J/pyunits.mol/pyunits.K)
        self.cp0_Kaolinite = Param(initialize=246.14, units=pyunits.J/pyunits.mol/pyunits.K)
        
        self.cp1_Un2O3 = Param(initialize=0.17156, units=pyunits.J/pyunits.mol/pyunits.K**2)
        self.cp1_CaO = Param(initialize=0.0059258, units=pyunits.J/pyunits.mol/pyunits.K**2)
        self.cp1_SiO2 = Param(initialize=0.054286, units=pyunits.J/pyunits.mol/pyunits.K**2)
        self.cp1_Al2O3 = Param(initialize=0.17156, units=pyunits.J/pyunits.mol/pyunits.K**2)
        self.cp1_Fe2O3 = Param(initialize=0.09936, units=pyunits.J/pyunits.mol/pyunits.K**2)
        self.cp1_CaCO3 = Param(initialize=0.046296, units=pyunits.J/pyunits.mol/pyunits.K**2)
        self.cp1_Pyrite = Param(initialize=0.024232, units=pyunits.J/pyunits.mol/pyunits.K**2)
        self.cp1_Kaolinite = Param(initialize=0.0, units=pyunits.J/pyunits.mol/pyunits.K**2)

        # standard heat of formation of impurity species
        self.enth0_comp_impurity = Param(self.impurity_list, mutable=True, units=pyunits.J/pyunits.mol)
        ...
        self.enth0_comp_product = Param(self.product_list, mutable=True, units=pyunits.J/pyunits.mol)
        ...
        self.cp0_comp_impurity = Param(self.impurity_list, mutable=True, units=pyunits.J/pyunits.mol/pyunits.K)
        ...
        self.cp0_comp_product = Param(self.product_list, mutable=True, units=pyunits.J/pyunits.mol/pyunits.K)
        ...
        self.cp1_comp_impurity = Param(self.impurity_list, mutable=True, units=pyunits.J/pyunits.mol/pyunits.K**2)
        ...
        self.cp1_comp_product = Param(self.product_list, mutable=True, units=pyunits.J/pyunits.mol/pyunits.K**2)
        ...
        self.cp_organic = Param(initialize=1260, units=pyunits.J/pyunits.mol/pyunits.K)

        # unit constants used for the expressions of HHV and heat of formation of organic material and liquid water enthalpy
        self.enth_mas_const = Param(initialize=1, units=pyunits.J/pyunits.kg, doc='1 unit of mass enthalpy in J/kg')
    ...
    def _make_vars(self):
        ...
        self.mass_frac_organic_feed = Var(self.flowsheet().config.time, initialize= 0.1,
                        doc='mass fraction of total organic materials in solid feed')

        self.mass_frac_comp_impurity_feed = Var(self.flowsheet().config.time, self.impurity_list, initialize= 0.2,
                        doc='mass fraction of impurity minerals in solid feed stream on dry organic free basis')

        self.mass_frac_comp_organic_feed = Var(self.flowsheet().config.time, self.organic_ele_list, initialize= 0.25,
                        doc='mass fraction of organic elements in solid feed stream on dry ash free basis')

        self.ppm_comp_ree_ins_feed = Var(self.flowsheet().config.time, self.ree_list, initialize=10,
                        doc='fraction of insoluble rare earth element in ppm per unit mass of impurity minerals in solid feed')

        self.ppm_comp_ree_dis_feed = Var(self.flowsheet().config.time, self.ree_list, initialize=10,
                        doc='mass fraction of dissovable rare earth element in ppm per unit mass of impurity minerals in solid feed')

        self.xconv_comp_ins = Var(self.flowsheet().config.time, self.ree_list, initialize=0.9,
                        doc='fraction of converson of insoluble REE to dissovable REE')

        self.frac_comp_ree_recovery = Var(self.flowsheet().config.time, self.ree_list, initialize=0.95,
                        doc='fraction of recovery of each REE in the product stream')

        self.frac_impurity_recovery = Var(self.flowsheet().config.time, initialize=0.95,
        ...
        self.xconv_caco3 = Var(self.flowsheet().config.time,
                        initialize=0.5,
                        doc='fractional conversion of CaCO3 calcination reaction')

        self.flow_mol_comp_product_total = Var(self.flowsheet().config.time, self.product_list, initialize=10,
                        units=pyunits.mol/pyunits.s,
                        doc='mole flow rate of total solid product from calcination of impurity minerals')

        self.flow_mol_comp_product_recovered = Var(self.flowsheet().config.time, self.product_list, initialize=10,
                        units=pyunits.mol/pyunits.s,
                        doc='mole flow rate of recovered solid product from calcination of impurity minerals')

        self.flow_mol_comp_product_dust = Var(self.flowsheet().config.time, self.product_list, initialize=1,
                        units=pyunits.mol/pyunits.s,
                        doc='mole flow rate of solid product in dust stream from calcination of impurity minerals')

        self.flow_mas_product_recovered = Var(self.flowsheet().config.time, initialize=1,
                        units=pyunits.kg/pyunits.s,
                        doc='mass flow rate of recovered solid product from calcination of impurity minerals')

        self.flow_mas_product_dust = Var(self.flowsheet().config.time, initialize=0.1,
                        units=pyunits.kg/pyunits.s,
                        doc='mass flow rate of solid product in dust stream from calcination of impurity minerals')

        self.mass_frac_comp_impurity_ele_product = Var(self.flowsheet().config.time, self.impurity_ele_list, initialize=0.1,
                        doc='mass fraction of impurity element in solid product stream')

        self.ppm_comp_ree_ins_product = Var(self.flowsheet().config.time, self.ree_list, initialize=10,
                        doc='fraction of insoluble rare earth element in product stream')

        self.ppm_comp_ree_dis_product = Var(self.flowsheet().config.time, self.ree_list, initialize=10,
    ...
    def _make_mass_balance(self):
        ...
        @self.Expression(self.flowsheet().config.time, doc="Organic material mass flowrate [kg/s]")
        def flow_mas_organic_feed(b, t):
            return b.flow_mas_feed[t]*b.mass_frac_organic_feed[t]

        # Organic element mole flow rate
        @self.Expression(self.flowsheet().config.time, self.organic_ele_list, doc="Organic material mass flowrate [kg/s]")
        def flow_mol_comp_organic_feed(b, t, i):
            return b.flow_mas_organic_feed[t]*b.mass_frac_comp_organic_feed[t,i]/b.am_comp_organic_ele[i]

        # impurity mineral mass flow rate
        @self.Expression(self.flowsheet().config.time, doc="Impurity mineral material mass flowrate [kg/s]")
        def flow_mas_impurity_feed(b, t):
            return b.flow_mas_feed[t]*(1-b.mass_frac_moist_feed[t]-b.mass_frac_organic_feed[t])

        # mass flow rates of individual mineral impurities
        @self.Expression(self.flowsheet().config.time, self.impurity_list, doc="mass flow rate of individual impurity minerals [kg/s]")
        ...
        @self.Expression(self.flowsheet().config.time, self.impurity_list, doc="mole flow rate of individual impurity minerals [mol/s]")
        ...
        @self.Constraint(self.flowsheet().config.time, self.config.gas_property_package.component_list, \
        ...
        @self.Constraint(self.flowsheet().config.time, self.product_list, doc="mole flow rate of total product before dust carry over")
        def flow_mol_comp_product_total_eqn(b, t, i):
            if i == "CaCO3":
                return b.flow_mol_comp_product_total[t,i] == b.flow_mol_comp_impurity_feed[t,'CaCO3']*(1-b.xconv_caco3[t])
            elif i == "CaO":
                return b.flow_mol_comp_product_total[t,i] == b.flow_mol_comp_impurity_feed[t,'CaCO3']*b.xconv_caco3[t]
            elif i == "SiO2":
                return b.flow_mol_comp_product_total[t,i] == b.flow_mol_comp_impurity_feed[t,'SiO2']\
                                                           + b.flow_mol_comp_impurity_feed[t,'Kaolinite']*2
            elif i == "Al2O3":
                return b.flow_mol_comp_product_total[t,i] == b.flow_mol_comp_impurity_feed[t,'Al2O3']\
                                                           + b.flow_mol_comp_impurity_feed[t,'Kaolinite']
            elif i == "Fe2O3":
                return b.flow_mol_comp_product_total[t,i] == b.flow_mol_comp_impurity_feed[t,'Pyrite']/2
            else: # i == "Un2O3"
                return b.flow_mol_comp_product_total[t,i] == b.flow_mol_comp_impurity_feed[t,'Un2O3']

        # mole flow rate of recovered solid product stream
        @self.Constraint(self.flowsheet().config.time, self.product_list, doc="mole flow rate of recovered product stream")
        def flow_mol_comp_product_recovered_eqn(b, t, i):
                return b.flow_mol_comp_product_recovered[t,i] == b.flow_mol_comp_product_total[t,i]*b.frac_impurity_recovery[t]

        # mole flow rate of solid product in dust stream
        @self.Constraint(self.flowsheet().config.time, self.product_list, doc="mole flow rate of recovered product stream")
        def flow_mol_comp_product_dust_eqn(b, t, i):
                return b.flow_mol_comp_product_dust[t,i] == b.flow_mol_comp_product_total[t,i]*(1-b.frac_impurity_recovery[t])
        ...
        def flow_mas_product_recovered_eqn(b, t):
            return b.flow_mas_product_recovered[t] \
                   == sum(b.flow_mol_comp_product_recovered[t,i]*b.mw_comp_product[i] for i in b.product_list)

        # mass flow rate of solid product in dust stream
        @self.Constraint(self.flowsheet().config.time, doc="mass flow rate of product")
        def flow_mas_product_dust_eqn(b, t):
            return b.flow_mas_product_dust[t] == sum(b.flow_mol_comp_product_dust[t,i]*b.mw_comp_product[i] for i in b.product_list)

        # mass fraction of impurity metal in the solid feed stream on dry organic free basis
        @self.Expression(self.flowsheet().config.time, self.impurity_ele_list, \
                         doc="mass fraction of impurity element in dry organic free solid feed stream")
        def mass_frac_comp_impurity_ele_feed(b, t, i):
            if i=='Ca':
                return b.flow_mol_comp_impurity_feed[t,'CaCO3']*b.am_Ca/b.flow_mas_impurity_feed[t]
            elif i=='Si':
                return (b.flow_mol_comp_impurity_feed[t,'SiO2']+b.flow_mol_comp_impurity_feed[t,'Kaolinite']*2)\
                       *b.am_Si/b.flow_mas_impurity_feed[t]
            elif i=='Al':
                return (b.flow_mol_comp_impurity_feed[t,'Al2O3']+b.flow_mol_comp_impurity_feed[t,'Kaolinite'])\
                       *b.am_Al*2/b.flow_mas_impurity_feed[t]
            elif i=='Fe':
                return b.flow_mol_comp_impurity_feed[t,'Pyrite']*b.am_Fe/b.flow_mas_impurity_feed[t]
            else: # i==Un
                return b.flow_mol_comp_impurity_feed[t,'Un2O3']*b.am_Un*2/b.flow_mas_impurity_feed[t]

        # mass fraction of impurity metal in the product stream
        @self.Constraint(self.flowsheet().config.time, self.impurity_ele_list, \
                         doc="mass fraction of impurity element in solid product")
        def mass_frac_comp_impurity_ele_product_eqn(b, t, i):
            if i=='Ca':
                return b.mass_frac_comp_impurity_ele_product[t,i]*b.flow_mas_product_recovered[t] ==\
                      (b.flow_mol_comp_product_recovered[t,'CaCO3']+ b.flow_mol_comp_product_recovered[t,'CaO'])*b.am_Ca
            elif i=='Si':
                return b.mass_frac_comp_impurity_ele_product[t,i]*b.flow_mas_product_recovered[t] ==\
        ...
        @self.Constraint(self.flowsheet().config.time, self.ree_list, doc="insoluble solid ppm in solid product")
        def ppm_comp_ree_ins_product_eqn(b, t, i):
            return b.ppm_comp_ree_ins_product[t,i]*b.flow_mas_product_recovered[t] == b.flow_mas_impurity_feed[t]*\
                b.ppm_comp_ree_ins_feed[t,i]*(1-b.xconv_comp_ins[t,i])*b.frac_comp_ree_recovery[t,i]

        # dissovable solid ppm in the product stream
        @self.Constraint(self.flowsheet().config.time, self.ree_list, doc="dissovable solid ppm in solid product")
        def ppm_comp_ree_dis_product_eqn_eqn(b, t, i):
            return b.ppm_comp_ree_dis_product[t,i]*b.flow_mas_product_recovered[t] \
                == b.flow_mas_impurity_feed[t]*(b.ppm_comp_ree_ins_feed[t,i]*b.xconv_comp_ins[t,i] \
                 + b.ppm_comp_ree_dis_feed[t,i])*b.frac_comp_ree_recovery[t,i]

        # solid product with 13 species defined in CoalRefuseParameters
        @self.Constraint(self.flowsheet().config.time, self.config.solid_property_package.component_list, 
                        doc="component flow of outlet solid stream")
        def solid_outlet_comp_eqn(b, t, i):
            if i=="Al2O3" or i=="CaO" or i=="Fe2O3":
                return b.solid_out[t].mass_frac_comp[i]*b.solid_out[t].flow_mass \
                    == b.flow_mol_comp_product_recovered[t,i]*b.config.solid_property_package.mw[i]
            elif i=="inerts":
                return b.solid_out[t].mass_frac_comp[i]*b.solid_out[t].flow_mass \
                ...
                return b.solid_out[t].mass_frac_comp[i] == (b.ppm_comp_ree_ins_product[t,"Sc"]+b.ppm_comp_ree_dis_product[t,"Sc"])/1e6
            elif i=="Y2O3":
                return b.solid_out[t].mass_frac_comp[i] == (b.ppm_comp_ree_ins_product[t,"Y"]+b.ppm_comp_ree_dis_product[t,"Y"])/1e6
            elif i=="La2O3":
                return b.solid_out[t].mass_frac_comp[i] == (b.ppm_comp_ree_ins_product[t,"La"]+b.ppm_comp_ree_dis_product[t,"La"])/1e6
            elif i=="Ce2O3":
                return b.solid_out[t].mass_frac_comp[i] == (b.ppm_comp_ree_ins_product[t,"Ce"]+b.ppm_comp_ree_dis_product[t,"Ce"])/1e6
            elif i=="Pr2O3":
                return b.solid_out[t].mass_frac_comp[i] == (b.ppm_comp_ree_ins_product[t,"Pr"]+b.ppm_comp_ree_dis_product[t,"Pr"])/1e6
            elif i=="Nd2O3":
                return b.solid_out[t].mass_frac_comp[i] == (b.ppm_comp_ree_ins_product[t,"Nd"]+b.ppm_comp_ree_dis_product[t,"Nd"])/1e6
            elif i=="Sm2O3":
                return b.solid_out[t].mass_frac_comp[i] == (b.ppm_comp_ree_ins_product[t,"Sm"]+b.ppm_comp_ree_dis_product[t,"Sm"])/1e6
            elif i=="Gd2O3":
                return b.solid_out[t].mass_frac_comp[i] == (b.ppm_comp_ree_ins_product[t,"Gd"]+b.ppm_comp_ree_dis_product[t,"Gd"])/1e6
            elif i=="Dy2O3":
                return b.solid_out[t].mass_frac_comp[i] == (b.ppm_comp_ree_ins_product[t,"Dy"]+b.ppm_comp_ree_dis_product[t,"Dy"])/1e6
            else:
                pass

    def _make_energy_balance(self):
        # HHV based on Dulong's formula
        @self.Expression(self.flowsheet().config.time, doc="HHV based on Dulong's formula [J/kg]")
        def HHV_Dulong(b, t):
            return b.enth_mas_const*(232367.224*(145.44*b.mass_frac_comp_organic_feed[t, 'C']\
                   + 620*(b.mass_frac_comp_organic_feed[t, 'H']-b.mass_frac_comp_organic_feed[t, 'O']/8)\
                   + 41*b.mass_frac_comp_organic_feed[t, 'S']))

        @self.Expression(self.flowsheet().config.time, doc="Standard heat of formation of organic materials [J/kg]")
        def Hf_organic(b, t):
            dhcoal = -b.HHV_Dulong[t] + const.gas_constant*b.temp_ref/2*\
            (-b.mass_frac_comp_organic_feed[t, 'H']/2/b.am_H + b.mass_frac_comp_organic_feed[t, 'O']/b.am_O \
            + b.mass_frac_comp_organic_feed[t, 'N']/b.am_N)
            dhc = -b.enth_mol_const*94052*4.184*b.mass_frac_comp_organic_feed[t, 'C']/b.am_C
            dhh = -b.enth_mol_const*68317.4*4.184*b.mass_frac_comp_organic_feed[t, 'H']/b.am_H/2
            dhs = -b.enth_mol_const*70940*4.184*b.mass_frac_comp_organic_feed[t, 'S']/b.am_S
            return dhc + dhh + dhs - dhcoal

        # molar enthalpy of impurity minerals in solid feed
        @self.Expression(self.flowsheet().config.time, self.impurity_list, doc='molar enthalpy of individual impurity minerals')
        def enth_mol_comp_impurity_feed(b, t, i):
            return b.enth0_comp_impurity[i] + b.cp0_comp_impurity[i]*(b.temp_feed[t]-b.temp_ref) \
                   + 0.5*b.cp1_comp_impurity[i]*(b.temp_feed[t]*b.temp_feed[t]-b.temp_ref*b.temp_ref)

        # molar enthalpy of minerals in solid product
        @self.Expression(self.flowsheet().config.time, self.product_list, doc='molar enthalpy of individual product minerals')
        def enth_mol_comp_product(b, t, i):
            temp_prod = b.gas_out[t].temperature
            return b.enth0_comp_product[i] + b.cp0_comp_product[i]*(temp_prod-b.temp_ref) \
                   + 0.5*b.cp1_comp_product[i]*(temp_prod*temp_prod-b.temp_ref*b.temp_ref)
        ...
        def energy_balance_eqn(b, t):
            if self.config.has_heat_transfer is True:
                ...
                heat = 0
            return sum(b.enth_mol_comp_impurity_feed[t,i]*b.flow_mol_comp_impurity_feed[t,i] for i in b.impurity_list) \
                   + b.flow_mol_moist_feed[t]*b.enth_mol_moist_feed[t]\
                   + b.flow_mas_organic_feed[t]*(b.Hf_organic[t] \
                   + b.cp_organic*(b.temp_feed[t]-b.temp_ref))\
                   + b.gas_in[t].flow_mol*b.gas_in[t].enth_mol + heat\
                   == sum(b.enth_mol_comp_product[t,i]*b.flow_mol_comp_product_total[t,i] for i in b.product_list)\
