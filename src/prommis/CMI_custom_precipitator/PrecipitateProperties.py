...
class PrecipitateParameterData(PhysicalParameterBlock):
    ...
    CONFIG.declare(
        "precip_comp_list", ConfigValue(domain=list, description="List of precipitate components in process.")
    )
    CONFIG.declare(
        "precip_eq_rxn_logkeq_dict", ConfigValue(domain=dict, description="Dictionary of precipitation equilibrium reaction constants")
    )
    CONFIG.declare(
        "precip_eq_rxn_stoich_dict", ConfigValue(domain=dict, description="Dictionary of precipitate component stoichiometry for each reaction.")
    )
    ...
    def build(self):
        ...
        self.eq_rxn_set = Set(initialize=list(set(key for key in self.config.precip_eq_rxn_logkeq_dict.keys())))
