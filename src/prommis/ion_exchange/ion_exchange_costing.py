...
"""This is modified version of the cost unit model for the ion
exchange unit that was developed to support the multicomponent ion
exchange unit model.
...
def build_h2so4_cost_param_block(blk):

    blk.cost = pyo.Param(
        mutable=True,
        initialize=0.119,
        doc="H2SO4 cost",  # from https://www.intratec.us/chemical-markets/sulfuric-acid-price
        units=pyo.units.USD_2019 / pyo.units.kg,
    )
    blk.purity = pyo.Param(
        mutable=True,
        initialize=1,
    ...
    costing = blk.parent_block()
    costing.register_flow_type("H2SO4", blk.cost / blk.purity)
...
def build_ion_exchange_cost_param_block(blk):
    ...
    blk.cation_exchange_resin_cost = pyo.Var(
        # # Tentative price for a "chelating amino phosphonic
        # # resin". Taking 5 USD/liter as the base. Note that the units
        # # should be in USD_2024/ft3. From:
        # # https://www.alibaba.com/product-detail/Chelating-amino-phosphonic-resins-for-second_1600071099994.html
        # initialize=198.21,
        # units=pyo.units.USD_2024 / pyo.units.ft**3,
        # doc="Cation exchange resin cost per cubic ft. Assumes a chelating amino phosphonic resin.",
        initialize=153,
        units=pyo.units.USD_2020 / pyo.units.ft**3,
...
@register_costing_parameter_block(
    build_rule=build_h2so4_cost_param_block,
    parameter_block_name="h2so4",
)
@register_costing_parameter_block(
