...
class diafiltration_model:
    ...
    def __init__(
            ...
            perc_precipitate={'permeate': {'Li': 0.81, 'Co': 0.05, 'Ni': 0.05},
        ...
        if mixing == 'tube':
            ...
            for i in pyo.RangeSet(self.ns):
                ...
                if i == self.ns:
                    ...
                    for sol in self.solutes:
                        m.fs.inlet_mixers[i].recycle.mass_solute[0, sol]\
                                                .fix(1e-8)
    ...
    def add_diafiltrate(self, m):
        ...
        for sol in self.solutes:
            m.fs.split_diafiltrate.inlet.mass_solute[0, sol]\
                                        .fix(self.diaf[sol])
    ...
    def add_objectives(self, m):
        ...
        def rec_perc_co(b):
            return (
                b.rec_mass_co / ((self.feed['Co']+self.diaf['Co'])
                                 * pyo.units.kg / pyo.units.hour)
            )

        @m.Expression()
        def rec_perc_li(b):
            return (
                b.rec_mass_li / ((self.feed['Li']+self.diaf['Li'])
                                 * pyo.units.kg / pyo.units.hour)
            )

        # mass recovery objectives
        m.co_obj = pyo.Objective(
            expr=m.rec_mass_co,
            sense=pyo.maximize
        )

        m.li_obj = pyo.Objective(
            expr=m.rec_mass_li,
            sense=pyo.maximize
        )
    ...
    def connect_diafiltrate(self, m):
        ...
        m.fs.split_precipitate_recycle.split_fraction[0, 'recycle']\
                                      .fix(self.diaf['solvent']
                                           / (self.feed['solvent']
                                              + self.diaf['solvent']))
    ...
    def precipitate_objectives(self, m):
        ...
        def prec_mass_co(b):
            return (
                m.fs.precipitator['retentate'].solid.mass_solute[0, 'Co']
            )

        @m.Expression()
        def prec_mass_li(b):
            return (
                m.fs.precipitator['permeate'].solid.mass_solute[0, 'Li']
            )
        ...
        def prec_perc_li(b):
            return (
                b.prec_mass_li / ((self.feed['Li'])
                                  * pyo.units.kg / pyo.units.hour)
            )
        m.prec_co_obj = pyo.Objective(
            expr=m.prec_mass_co,
            sense=pyo.maximize
        )
        m.prec_li_obj = pyo.Objective(
            expr=m.prec_mass_li,
            sense=pyo.maximize
        )
        m.prec_li_obj.deactivate()
        m.prec_li_lb = pyo.Constraint(
            expr=m.prec_perc_li >= m.R
        )
        m.Rco = pyo.Param(initialize=0.95, mutable=True)
        m.prec_co_lb = pyo.Constraint(
            expr=m.prec_perc_co >= m.Rco
        )
        ...
        if i != 1:
            if mixing == 'tube':
                ...
                for j in pyo.RangeSet(self.nt-1):
                    m.fs.recycle_splitters[i].split_fraction[0, f"outlet_{j}"]\
                                             .fix(1/self.nt)
                ...
                for j in pyo.RangeSet(self.nt):
                    ...
                    for sol in self.solutes:
                        m.fs.inlet_mixers[i-1, j].recycle.mass_solute[0, sol]\
                                                    .unfix()
    ...
    def initialize_single_stage(self, m, stage, mixing='tube'):
        ...
        for j in pyo.RangeSet(self.nt):
            ...
            for sol in self.solutes:
                m.fs.stage[i].retentate_side_stream_state[0, j]\
                             .mass_solute[sol]\
                             .unfix()
            if mixing == 'tube':
                propagate_state(
            ...
            # than the LB of 1e-8.
            m.fs.stage[i].material_transfer_term[0, j, 'permeate', 'retentate',
                                                 'solvent'].set_value(1e-4)
            # m.fs.stage[i].permeate[0.0, j].flow_vol.setlb(0)
            for sol in self.solutes:
                m.fs.stage[i].material_transfer_term[0, j, 'permeate',
                                                     'retentate',
                                                     sol].set_value(1e-4)
    ...
    def initialize_feeds(self, m, mixing):
        ...
        for i in pyo.RangeSet(inlets-1):
            ...
            m.fs.split_diafiltrate.split_fraction[0, f"outlet_{i}"]\
                                  .fix(1/inlets)
        split_initializer = SeparatorInitializer()
    ...
    def initialize(self, m, mixing='tube', precipitate=False):
        ...
        # TODO could incoporate initialization info into idaes logger
        print('*'*50)
        ...
        while not np.isclose(diaf_old, diaf_new) and itr < 100:
            diaf_old = m.fs.split_diafiltrate.mixed_state[0]\
                                             .mass_solute[check_sol].value
            ...
            while not np.isclose(check_old, check_new) and iteration < 100:
                ...
                for i in m.fs.inlet_mixers:
                    ...
                    if iteration == 0:
                        ...
                        if (m.fs.inlet_mixers[i].recycle.flow_vol[0].value
                                != 1e-8):
                            m.fs.inlet_mixers[i].recycle.flow_vol[0].fix(0)
                            for sol in self.solutes:
                                m.fs.inlet_mixers[i].recycle\
                                                    .mass_solute[0, sol]\
                                                    .fix(0)
                    else:
                        if i == check_loc:
                            check_old = m.fs.inlet_mixers[i].mixed_state[0]\
                                                            .flow_vol.value

                    mixer_initializer = MixerInitializer()
                    mixer_initializer.initialize(m.fs.inlet_mixers[i])
                    if i == check_loc:
                        check_new = m.fs.inlet_mixers[i].mixed_state[0]\
                                                        .flow_vol.value
                ...
                # initialize stages and relevent splitters
                for i in pyo.RangeSet(self.ns):
            ...
            diaf_new = m.fs.split_diafiltrate.mixed_state[0]\
                                             .mass_solute[check_sol].value
    ...
    def unfix_dof(self, m, mixing='tube', precipitate=False):
        ...
        for i in pyo.RangeSet(self.ns):
            for j in pyo.RangeSet(self.nt):
                m.fs.stage[i].retentate_side_stream_state[0, j].flow_vol\
                                                               .unfix()
                for sol in self.solutes:
                    m.fs.stage[i].retentate_side_stream_state[0, j]\
                                .mass_solute[sol].unfix()
