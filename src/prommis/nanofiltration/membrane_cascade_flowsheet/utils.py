...
def report_values(m, prec=True):
    ...
    data['Membrane outlet recoveries'] = [pyo.value(m.rec_perc_co), pyo.value(m.rec_perc_li)]
    print('Membrane Area')
    data['Membrane Area'] = []
    ...
    if prec:
        ...
        for sol in m.fs.solutes:
            ...
            data['prec sf retentate'].append(m.fs.precipitator['retentate'].yields[sol, 'solid'].value)
        print('^retentate vpermeate')
        data['prec sf permeate'] = []
        for sol in m.fs.solutes:
            print(m.fs.precipitator['permeate'].yields[sol, 'solid'].value)
            data['prec sf permeate'].append(m.fs.precipitator['permeate'].yields[sol, 'solid'].value)
...
def visualize_flows(num_boxes, num_sub_boxes, conf='stage', model=None):
    ...
    fig, ax = plt.subplots(figsize=(2*(num_boxes * (box_size + spacing)),2*(box_size + 1)))
    ...
    for i in range(num_boxes):
        ...
        if i != num_boxes - 1:
            ...
            ax.arrow(
                x_start + box_size + (spacing)/2,  # Start X position (shifted further right)
                y_start + 0.05,  # Y position (above bottom border)
        ...
        if conf != 'classic':
            if i != 0 and model['Co product flows'][i*3] > tol:
                ax.arrow(
                    x_start + box_size + 0.03,  # Start X position (shifted further right)
                    y_start + box_size - 0.02 + prodlength,  # Y position (above bottom border)
                    0,  # X direction (shorter arrow)
    ...
    for x_start, y_start, sub_box_width, box_size, j, i in arrow_positions:
        if conf == 'tube':
            ...
            ax.arrow(
                ...
                y_start + box_size + 0.15,  # Start Y position (slightly above middle rect)
                0,  # X direction
            ...
            ax.add_patch(patches.Polygon([[xloc, yloc],
            ...
            if i != num_boxes - 1:
                if model['recycle flows'][(num_sub_boxes)*i + j] > tol:
                    ax.arrow(
                        ...
                        yloc + 0.23 + i*0.1,  # Start Y position (slightly above middle rect)
                        0,  # X direction
            ...
            if model['feed flows'][(num_sub_boxes)*i + j] > tol:
                ax.arrow(
                    x_start + j * sub_box_width + sub_box_width / 2,  # X position
                    yloc + (num_boxes+1)*0.1,  # Start Y position (slightly above middle rect)
                    0,  # X direction
            ...
            if j == 0:
                if i == 0 and model['feed flows'][(num_sub_boxes)*i + j] > tol:
                    ax.plot([xloc, 0], [yloc + (num_boxes+1)*0.1, yloc + (num_boxes+1)*0.1], c=to_rgba('red'))
            ...
            if model['diaf flows'][(num_sub_boxes)*i + j] > tol:
                ax.arrow(
                    xloc + sub_box_width/2-sub_box_width/8,  # X position
                    yloc + 0.06 + (num_boxes+1)*0.1,  # Start Y position (slightly above middle rect)
                    0,  # X direction
        ...
        ax.arrow(
            x_start + j * sub_box_width + sub_box_width / 2,  # X position
            y_start + box_size / 2 + 0.07,  # Start Y position (slightly above middle rect)
            0,  # X direction
        ...
        ax.arrow(
            x_start + j * sub_box_width + 0.5 * sub_box_width,  # Start X position (shifted further right)
