...
def test_parameters(model):
    ...
    for j in model.fs.feed_properties.component_list:
        ...
        assert j in model.fs.feed_properties.charge
        assert j in model.fs.feed_properties.molar_mass
        assert j in model.fs.feed_properties.sigma

    assert isinstance(model.fs.feed_properties.num_solutes, Param)
    assert value(model.fs.feed_properties.num_solutes) == 5
