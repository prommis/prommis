...
#####################################################################################################
"""
property package for the solid product stream of a feedstock roaster
...
# -----------------------------------------------------------------------------
@declare_process_block_class("ReeRoastParameters")
class ReeRoastParameterData(PhysicalParameterBlock):
    """
    Solid phase property package for feed roaster product.
    ...
    * Gangue Minerals: Kaolinite, Al2O3, SiO2, CaCO3, CaO, FeS2, Fe2O3
    ...
    def build(self):
        ...
        self._state_block_class = ReeRoastStateBlock
        ...
        obj.add_properties(
            {
                ...
                "mass_frac_ins": {"method": None, "units": "kg/kg"},
            }
        )
...
class _ReeRoastStateBlock(StateBlock):
...
@declare_process_block_class("ReeRoastStateBlock", block_class=_ReeRoastStateBlock)
class ReeRoastStateBlockData(StateBlockData):
    """
    ...
    def build(self):
        ...
        def rule_cp_mol_comp(b, i):
            return self.params.cp0_comp[i] + self.params.cp1_comp[i] * b.temperature

        self.cp_mol_comp = Expression(self.params.component_list, rule=rule_cp_mol_comp)

        def rule_cp_mol(b):
            return sum(
                b.cp_mol_comp[i] * b.mole_frac_comp[i]
                for i in self.params.component_list
            )
        ...
        def rule_cp_mass(b):
            return sum(
                b.cp_mol_comp[i] * b.mass_frac_comp[i] / self.params.mw_comp[i]
                for i in self.params.component_list
            )
        ...
        def rule_mass_frac_ins(b):
            return (
                b.mass_frac_comp["Ree2X"]
                + b.mass_frac_comp["Sc2X"]
                + b.mass_frac_comp["Y2X"]
                + b.mass_frac_comp["La2X"]
                + b.mass_frac_comp["Ce2X"]
                + b.mass_frac_comp["Pr2X"]
                + b.mass_frac_comp["Nd2X"]
                + b.mass_frac_comp["Sm2X"]
                + b.mass_frac_comp["Gd2X"]
                + b.mass_frac_comp["Dy2X"]
            )

        self.mass_frac_ins = Expression(rule=rule_mass_frac_ins)
        ...
        if not self.is_property_constructed("energy_density_terms"):
            try:
                ...
                def rule_energy_density_terms(b, p):
                    return self.enth_mass * self.params.dens_mass
