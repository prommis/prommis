...
def model():
    ...
    m.fs.roast_prop = ReeRoastParameters()
...
def test_parameters(model):
    assert len(model.fs.roast_prop.phase_list) == 1
    for k in model.fs.roast_prop.phase_list:
        assert k == "solid"

    for k in model.fs.roast_prop.component_list:
        assert k in [
            "Al2O3",
            "SiO2",
            "CaCO3",
        ...
        assert k in model.fs.roast_prop.mw_comp
        assert k in model.fs.roast_prop.enth0_comp
        assert k in model.fs.roast_prop.cp0_comp
...
def test_build_state(model):
    model.fs.state = model.fs.roast_prop.build_state_block(model.fs.time)
