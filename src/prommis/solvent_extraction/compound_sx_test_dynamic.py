...
m.fs.compound_solex.organic_inlet.conc_mass_comp[:, "DEHPA"].fix(975.8e3 * dosage / 100)
...
def volume_fraction_rule(m, t, s):
    ...
    theta_A = m.fs.compound_solex.solvent_extraction_mixer[
        s
    ].unit.mscontactor.volume_frac_stream[t, 1, "aqueous"]
    theta_O = m.fs.compound_solex.solvent_extraction_mixer[
        s
    ].unit.mscontactor.volume_frac_stream[t, 1, "organic"]
    v_A = (
        m.fs.compound_solex.solvent_extraction_mixer[s]
        .unit.mscontactor.aqueous[t, 1]
        .flow_vol
    )
    v_O = (
        m.fs.compound_solex.solvent_extraction_mixer[s]
        .unit.mscontactor.organic[t, 1]
        .flow_vol
    )
...
m.fs.compound_solex.solvent_extraction_mixer[
    :
].unit.mscontactor.aqueous_inherent_reaction_extent[0.0, :, "Ka2"].fix()
...
m.fs.compound_solex.solvent_extraction_mixer[:].unit.mscontactor.organic[
    0, :
].conc_mass_comp["DEHPA"].fix()

for e in m.fs.reaxn.element_list:
    m.fs.compound_solex.solvent_extraction_mixer[
        :
    ].unit.mscontactor.heterogeneous_reaction_extent[0.0, :, f"{e}_mass_transfer"].fix()
...
m.fs.compound_solex.solvent_extraction_aqueous_settler[:].unit.length.fix(0.4)
m.fs.compound_solex.solvent_extraction_organic_settler[:].unit.length.fix(0.4)
...
for s in m.fs.compound_solex.elements:
    for x in m.fs.compound_solex.solvent_extraction_aqueous_settler[
        ...
        if x != 0:
            for e in m.fs.leach_soln.component_list:
                if e not in ["H2O", "HSO4"]:
                    m.fs.compound_solex.solvent_extraction_aqueous_settler[
                        s
                    ].unit.properties[0, x].conc_mass_comp[e].fix()
            ...
            m.fs.compound_solex.solvent_extraction_aqueous_settler[
                s
            ].unit.inherent_reaction_extent[0, x, "Ka2"].fix()
            P = value(
    ...
    for x in m.fs.compound_solex.solvent_extraction_organic_settler[
        ...
        if x != 0:
            for e in m.fs.prop_o.component_list:
                if e != "Kerosene":
                    m.fs.compound_solex.solvent_extraction_organic_settler[
                        s
                    ].unit.properties[0, x].conc_mass_comp[e].fix()
...
# #             1
# #             - (
# #                 m.fs.compound_solex.solvent_extraction_aqueous_settler[number_of_stages]
# #                 .unit.properties[t, 1]
# #                 .conc_mass_comp[e]()
# #                 * m.fs.compound_solex.solvent_extraction_aqueous_settler[
# #                     number_of_stages
...
# plt.plot(
#     m.fs.time,
#     m.fs.compound_solex.solvent_extraction_mixer[1]
#     .unit.mscontactor.aqueous[:, :]
#     .conc_mass_comp["Y"](),
# )
# plt.axvline(x=4, color="red", linestyle="--")
# plt.xlabel("Time, hrs")
# plt.ylabel("Y aqueous concentration mixer 1, mg/L")
# plt.title("Y mixer 1 aqueous phase profile")
# plt.show()

# plt.plot(
#     m.fs.time,
#     m.fs.compound_solex.solvent_extraction_aqueous_settler[1]
#     .unit.properties[:, 0]
#     .conc_mass_comp["Y"](),
# )
# plt.axvline(x=4, color="red", linestyle="--")
# plt.xlabel("Time, hrs")
# plt.ylabel("Y aqueous concentration settler 1, mg/L")
# plt.title("Y settler 1 aqueous phase profile at x=0")
# plt.show()
...
# plt.xlabel("Time, hrs")
# plt.ylabel("Y aqueous concentration settler 1, mg/L")
# plt.title("Y settler 1 aqueous phase profile at x=1")
