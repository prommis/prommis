...
def _validate_distribution_coefficients(unit, metal_list, expr_dict):
    for j in metal_list:
        D_mean = 1
...
class Test_Solvent_Extraction_steady_model:
    ...
    def test_get_performance_contents(self, SolEx_frame):
        ...
        out = out["exprs"]
        assert len(out) == len(self.metal_list)

        _validate_distribution_coefficients(unit, self.metal_list, out)

    def test_get_performance_contents_loading(self, SolEx_frame):
        unit = SolEx_frame.fs.solex
        unit.config.extraction_direction = ExtractionDirection.loading

        out = unit._get_performance_contents()
        assert len(out) == 1

        out = out["exprs"]
        assert len(out) == 2 * len(self.metal_list)

        _validate_distribution_coefficients(unit, self.metal_list, out)

        for j in self.metal_list:
            recovery = 100 * value(
                unit.mscontactor.organic[0, 1].flow_mol_comp[j]
                / unit.mscontactor.aqueous_inlet_state[0].flow_mol_comp[j]
            )
            assert out[f"Recovery % {j}"] == pytest.approx(
                recovery, rel=1e-12, abs=1e-12
            )
