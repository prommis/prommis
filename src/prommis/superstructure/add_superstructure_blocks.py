...
#####################################################################################################
"""
Add Superstructure Blocks Code
==============================
...
def add_costing_vars(m):
    ...
    **References:**
    ...
    m.fs.costing.main_product_profit = pyo.Var(
        ...
        domain=pyo.NonNegativeReals,
        bounds=(0, 1e9),
        doc="The total profit generated from the main product each year.",
        units=pyunits.USD / pyunits.year,
    )
...
def add_byproduct_valorization_params(m, byproduct_values, byproduct_opt_conversions):
    ...
    m.fs.byproduct_valorization.byproducts_set = pyo.Set(
        initialize=list(byproducts), doc="Set of byproducts considered."
    )
    ...
    def calculate_byproduct_produced_cons(b, byprod, t):
        return m.fs.byproduct_valorization.byproduct_produced[byprod, t] == sum(
            ...
            * m.fs.byproduct_valorization.byproduct_opt_conversion[opt, byprod]
            ...
            if (opt, byprod) in m.fs.byproduct_valorization.opt_byproduct_set
        )
...
def add_profit_cons(m, consider_byproduct_valorization: bool):
    ...
    **References:**
    ...
    if (
        ...
        @m.fs.costing.Constraint(
            m.fs.operational_range,
            doc="Calculates the yearly profit from the main product of the plant.",
        )
        def calculate_main_product_profit_cons(b, t):
            return m.fs.costing.main_product_profit[t] == sum(
        ...
        def calculate_main_product_profit_cons(b, t):
            return m.fs.costing.main_product_profit[
                t
        ...
        if (
            ...
            def calculate_total_profit_cons(b, t):
                return (
                    ...
                    == m.fs.costing.main_product_profit[t]
    ...
    # don't include profit from byproducts
        ...
        if (
            ...
            def calculate_total_profit_cons(b, t):
                return (
                    m.fs.costing.total_profit[t] == m.fs.costing.main_product_profit[t]
                )
...
def add_environmental_impact_params(
    ...
    *Applied Mathematics and Computation*, 213:455-465, 2009.
    ...
    m.fs.environmental_impacts.options_environmental_impacts = pyo.Param(
        ...
        doc="Environmental impacts matrix. Units of chosen indicator per kg of incoming flowrate.",
        units=1 / pyunits.kg,
    )
