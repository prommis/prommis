...
def add_plant_lifetime_params_block(m, plant_start, plant_lifetime):
    ...
    m.plant_lifetime_params.plant_start = pyo.Param(initialize=plant_start, doc="The year that plant construction begins.")
    m.plant_lifetime_params.plant_lifetime = pyo.Param(initialize=plant_lifetime, doc="The total lifetime of the plant, including plant construction. Must be at least three years.")

    ## Calculate other necessary params from user input
    # first year is construction
    prod_start = plant_start + 1
    m.plant_lifetime_params.prod_start = pyo.Param(initialize=prod_start, doc="The first year of plant production.")
    # final year plant is in production
    plant_end = plant_start + plant_lifetime - 1
    m.plant_lifetime_params.plant_end = pyo.Param(initialize=plant_end, doc="The final year of plant production.")
    # lifetime of the plant
    m.plant_lifetime_params.plant_life_range = pyo.RangeSet(plant_start, plant_end, doc="Lifetime of the plant.")
    # operational lifetime of the plant
    m.plant_lifetime_params.operational_range = pyo.RangeSet(prod_start, plant_end, doc="Operational lifetime of the plant.")

    return m

###################################################################################################
### Feed Parameters
def check_feed_params(m, available_feed, collection_rate, tracked_comps, prod_comp_mass):
    """
    ...
    if any(value == 0 for value in prod_comp_mass.values()):
        ...
        warnings.warn(f"prod_comp_mass contains zero values for the tracked components: {zero_prods}. ")

def add_feed_params_block(m, available_feed, collection_rate, tracked_comps, prod_comp_mass):
    """
    ...
    m.feed_params.available_feed = pyo.Param(m.plant_lifetime_params.operational_range, initialize=available_feed, doc="The total feedstock available for recycling each year.")
    m.feed_params.collection_rate = pyo.Param(initialize=collection_rate, doc="The fraction of available feed that is processed by the plant each year.")
    m.feed_params.tracked_comps = pyo.Set(initialize=tracked_comps, doc="Tracked components.")
    m.feed_params.prod_comp_mass = pyo.Param(m.feed_params.tracked_comps, initialize=prod_comp_mass, doc="The mass of each tracked component per EOL product.")
    ...
    m.feed_params.feed_entering = pyo.Param(m.plant_lifetime_params.operational_range, initialize=feed_entering, doc="The amount of feed entering the plant each year.")
    
    # max feed entering plant over production period
    max_feed_entering = max(
        feed_entering.values()
    )
    m.feed_params.max_feed_entering = pyo.Param(initialize=max_feed_entering, doc="The max yearly feed that enters the plant over the production period.")

    # year in which max feed enters plant
    max_feed_entering_year = max(
        feed_entering, key=feed_entering.get
    )
    m.feed_params.max_feed_entering_year = pyo.Param(initialize=max_feed_entering_year, doc="The year that the max feed enters the plant.")

    return m

###################################################################################################
### Superstructure Formulation Parameters
def check_supe_formulation_params(m, num_stages, options_in_stage, option_outlets, option_eff):
    """
...
def add_supe_formulation_params(m, num_stages, options_in_stage, option_outlets, option_eff):
    """
    ...
    m.supe_form_params.num_stages = pyo.Param(initialize=num_stages, doc="The total number of stages in the superstructure.")
    m.supe_form_params.stages_set = pyo.RangeSet(1, num_stages, doc="A set of all the stages in the superstructure.")
    m.supe_form_params.options_in_stage = pyo.Param(m.supe_form_params.stages_set, initialize=options_in_stage, doc="The number of options in each stage.")
    
    # Define a parameter for the max number of options in any of the stages
    max_options = max(options_in_stage.values())
    m.supe_form_params.max_options = pyo.Param(initialize=max_options, doc="The max number of options in any of the stages.")
    m.supe_form_params.max_options_set = pyo.RangeSet(1, max_options, doc="Set containing max number of options in any of the stages.")
    ...
    m.supe_form_params.option_outlets = pyo.Param(m.supe_form_params.opt_set, initialize=option_outlets, doc="Defines the set of options k' in stage j+1 connected to option k in stage j.")
    ...
    m.supe_form_params.option_eff = pyo.Param(m.supe_form_params.opt_set, m.feed_params.tracked_comps, initialize=option_eff_initialize)

    # Define a set containing all of the options in the final stage
    final_opt_list = [(num_stages, k) for k in range(1, options_in_stage[num_stages] + 1)]
    m.supe_form_params.final_opt_set = pyo.Set(initialize=final_opt_list, doc="Set containing all of the options in the final stage.")

    return m

###################################################################################################
### Operating Parameters
def check_operating_params(m, profit, opt_var_oc_params, discr_opts, workers_per_discr_unit, yearly_cost_per_unit, cost_per_unit, processing_rate, num_workers, labor_rate):
    """
    ...
    if profit_opt_keys != final_opt_set:
        raise ValueError("Must include profit per unit of product for all options in the final stage.")
    ...
    if missing_profit_comps:
        msg = "Profits not specifed for all tracked components in the following options:\n"
        msg += "\n".join(
            f"  Option (stage={j}, option={k}) missing components: {missing}"
    ...
    if opt_var_oc_params_keys != continuous_opts_set:
        raise ValueError("Variable operating cost params not defined for all continuous options.")
    ...
    negative_workers_per_discr_units = [opt for opt, w in workers_per_discr_unit.items() if w < 0]
    ...
    if yearly_cost_per_unit_keys != discr_opts_set:
        raise ValueError("yearly_cost_per_unit must be defined for all discrete options.")
    
    ## Check that the yearly cost per unit values are all non-negative
    negative_yearly_cost_per_unit = [opt for opt, cost in yearly_cost_per_unit.items() if cost < 0]
    ...
    nonpositive_processing_rate = [opt for opt, rate in processing_rate.items() if rate <= 0]
    ...
    if num_workers_keys != continuous_opts_set:
        raise ValueError("num_workers must be defined for all continuous options, and must not be defined for discrete options.")
...
def build_model(
    ...
    # list of the options that utilitize discrete units (typically disassembly options)
    # of the form -> [(1, 1), (1, 2), ...]
    dis_opts: list,
    ...
    check_feed_parameters(m, available_feed, collection_rate, tracked_comps, prod_comp_mass)
    # Create separate block to hold feed parameters
    m = add_feed_params_block(m, available_feed, collection_rate, tracked_comps, prod_comp_mass)
