...
##############################################################################################################################
### Test EVs & HEVs Superstructructure
##############################################################################################################################
@pytest.fixture(scope="module")
def get_ev_common_params():
    return {
        ###################################################################################################
...
class TestNPV(object):
    ...
    def NPV_model(self, get_ev_common_params):
        common_params = get_ev_common_params
        m = build_model(
            ###################################################################################################
            ### Plant Lifetime Parameters
    ...
    def test_build(self, NPV_model, get_ev_common_params):
        # start of plant production
        prod_start = get_ev_common_params["plant_start"] + 1
        # final year plant is in production
        plant_end = (
            get_ev_common_params["plant_start"]
            + get_ev_common_params["plant_lifetime"]
            - 1
        )
    ...
    @pytest.mark.skipif(not solver_available, reason="Gurobi solver not available")
    @pytest.mark.component
    def test_solution(self, NPV_model, get_ev_common_params):
        ##### Define Parameters for tests
        ###################################################################################################
        ### Plant Lifetime Parameters
        plant_start = get_ev_common_params["plant_start"]  # start of plant production
        plant_lifetime = get_ev_common_params["plant_lifetime"]  # lifetime of plant
        ...
        ### Feed parameters
        # Total feedstock available for recycling each year
        Available_feed = get_ev_common_params["Available_feed"]
        # collection rate for how much of the available feed is processed by the plant each year (fraction)
        CR = get_ev_common_params["CR"]
        # list of tracked components
        Tracked_comps = get_ev_common_params["Tracked_comps"]
        # mass of tracked component per EOL Product (kg component / EOL product)
        Prod_comp_mass = get_ev_common_params["Prod_comp_mass"]
        ...
        ###################################################################################################
        ### Superstructure formulation parameters
        numStages = get_ev_common_params["numStages"]  # no. of total stages
        # number of options in each stage
        Options_in_stage = get_ev_common_params["Options_in_stage"]
        # set of options k' in stage j+1 connected to option k in stage j
        Option_outlets = get_ev_common_params["Option_outlets"]
        # dictionary of tracked component retention efficiency for each option
        Option_Eff = get_ev_common_params["Option_Eff"]
        ...
        ### Operating Parameters
        # profit per kg of product in terms of tracked components
        Profit = get_ev_common_params["Profit"]
        # conversion of kg REE/Fe to kg REO/Fe2O3
        REE_to_REO_Conversion = get_ev_common_params["REE_to_REO_Conversion"]
        # For all options excluding the disassembly stage, the OPEX costs are linearly related to the flow entering it.
        # OPEX = a*F_in + b*y
        N_OC_var = get_ev_common_params["N_OC_var"]
        # number of workers, and type, needed by option (for disassembly stage, its operators per unit)
        num_workers = get_ev_common_params["num_workers"]
        labor_rate = get_ev_common_params["labor_rate"]  # yearly wage per type of labor
        # yearly operating costs per unit ($/unit*yr)
        YCU = get_ev_common_params["YCU"]
        # cost per disassembly stage unit for each disassembly option
        CU = get_ev_common_params["CU"]
        # disassembly rate for each disassembly option (in terms of EOL products disassembled per year per unit)
        Dis_Rate = get_ev_common_params["Dis_Rate"]
        ...
        ###################################################################################################
        ### Costing Parameters
        LF = get_ev_common_params["LF"]  # Lang Factor
        TOC_factor = get_ev_common_params["TOC_factor"]  # Overnight costs factor
        ATWACC = get_ev_common_params["ATWACC"]  # discount rate
        i_OC_esc = get_ev_common_params["i_OC_esc"]  # opex, revenue escalation rate
        i_CAP_esc = get_ev_common_params["i_CAP_esc"]  # capex escalation rate
        f_exp = get_ev_common_params["f_exp"]  # capital expenditure schedule
        # Define Python Dictionary with discretized cost by flows for each option.
        Discretized_CAPEX = get_ev_common_params["Discretized_CAPEX"]
        ###################################################################################################
        ###################################################################################################
...
class TestCOR(object):
    @pytest.fixture(scope="class")
    def COR_model(self, get_ev_common_params):
        common_params = get_ev_common_params
    ...
    def test_build(self, COR_model, get_ev_common_params):
        # start of plant production
        prod_start = get_ev_common_params["plant_start"] + 1
        # final year plant is in production
        plant_end = (
    ...
    @pytest.mark.skipif(not solver_available, reason="Gurobi solver not available")
    @pytest.mark.component
    def test_solution(self, COR_model, get_ev_common_params):
        ##### Define Parameters for tests
        ###################################################################################################
        ### Plant Lifetime Parameters
        plant_start = get_ev_common_params["plant_start"]  # start of plant production
...
class TestEnvironmentalImpacts(object):
    @pytest.fixture(scope="class")
    def EI_model(self, get_ev_common_params):
        common_params = get_ev_common_params
        m = build_model(
            ###################################################################################################
            ### Plant Lifetime Parameters
    ...
    def test_build(self, EI_model, get_ev_common_params):
        # start of plant production
        prod_start = get_ev_common_params["plant_start"] + 1
        # final year plant is in production
        plant_end = (
        ...
        operational_range = RangeSet(prod_start, plant_end)
        # no. of total stages
        numStages = get_ev_common_params["numStages"]
        # number of options in each stage
        Options_in_stage = get_ev_common_params["Options_in_stage"]
    ...
    @pytest.mark.skipif(not solver_available, reason="Gurobi solver not available")
    @pytest.mark.component
    def test_solution(self, EI_model, get_ev_common_params):
        ##### Define Parameters for tests
        ###################################################################################################
        ### Plant Lifetime Parameters
        plant_start = get_ev_common_params["plant_start"]  # start of plant production
        ...
        ###################################################################################################
        ### Consideration of environmental impacts parameters
        environ_impacts = get_ev_common_params["environ_impacts"]
        ###################################################################################################
        ###################################################################################################
        ### Other Parameters needed for tests
        # make a list of all options
...
class TestByprodVal(object):
    @pytest.fixture(scope="class")
    def BV_model(self, get_ev_common_params):
        common_params = get_ev_common_params
        m = build_model(
            ###################################################################################################
            ### Plant Lifetime Parameters
    ...
    def test_build(self, BV_model, get_ev_common_params):
        # start of plant production
        prod_start = get_ev_common_params["plant_start"] + 1
        # final year plant is in production
        plant_end = (
    ...
    @pytest.mark.skipif(not solver_available, reason="Gurobi solver not available")
    @pytest.mark.component
    def test_solution(self, BV_model, get_ev_common_params):
        ##### Define Parameters for tests
        ###################################################################################################
        ### Plant Lifetime Parameters
        plant_start = get_ev_common_params["plant_start"]  # start of plant production
        ...
        ### Byproduct valorization
        # list of byproducts
        byprods = get_ev_common_params["byprods"]
        # dictionary of values for each byproduct ($/kg). Negative value indicates it cost money to dispose of the byproduct
        byprod_vals = get_ev_common_params["byprod_vals"]
        # dictionary keeping track of which tracked component produces which byproduct
        tracked_comp_for_byprod = get_ev_common_params["tracked_comp_for_byprod"]
        # dictionary tracking which options produce a given byproduct
        byprod_options = get_ev_common_params["byprod_options"]
        # dictionary tracking byproduct recovery efficiency for each option (in terms of tracked component)
        byprod_options_eff = get_ev_common_params["byprod_options_eff"]
        # Conversion factors of tracked component to byproduct (kg byproduct / kg iron)
        TC_to_byproduct = get_ev_common_params["TC_to_byproduct"]
        ###################################################################################################
        ###################################################################################################
        ### Other Parameters needed for tests
        # make a list of all options
...
class TestNoObjectiveFunction(object):
    def test_no_objective_chosen(self, get_ev_common_params):
        common_params = get_ev_common_params
        ...
        with pytest.raises(SystemExit) as excinfo:
            m = build_model(
                ...
                ###################################################################################################
                ### Choice of objective function. Options are 'NPV' or 'COR'.capitalize
                obj_func="wrong objective function",
                ###################################################################################################
                ###################################################################################################
                ### Consideration of environmental impacts parameters
                # boolean to decide whether or not to consider environmental impacts
...
##############################################################################################################################
### Test HDDs Superstructructure
##############################################################################################################################
@pytest.fixture(scope="module")
def get_hdd_common_params():
    return {
        ###################################################################################################
        ### Plant Lifetime Parameters
        "plant_start": 2024,  # start of plant production
...
class TestNPV(object):
    @pytest.fixture(scope="class")
    def NPV_model(self, get_hdd_common_params):
        common_params = get_hdd_common_params
        m = build_model(
            ###################################################################################################
            ### Plant Lifetime Parameters
            plant_start=common_params["plant_start"],  # start of plant production
    ...
    def test_build(self, NPV_model, get_hdd_common_params):
        # start of plant production
        prod_start = get_hdd_common_params["plant_start"] + 1
        # final year plant is in production
        plant_end = (
            get_hdd_common_params["plant_start"]
            + get_hdd_common_params["plant_lifetime"]
            - 1
        )
        # plant operational period
        operational_range = RangeSet(prod_start, plant_end)
    ...
    @pytest.mark.skipif(not solver_available, reason="Gurobi solver not available")
    @pytest.mark.component
    def test_solution(self, NPV_model, get_hdd_common_params):
        ##### Define Parameters for tests
        ###################################################################################################
        ### Plant Lifetime Parameters
        plant_start = get_hdd_common_params["plant_start"]  # start of plant production
        plant_lifetime = get_hdd_common_params["plant_lifetime"]  # lifetime of plant
        # first year is construction
        prod_start = plant_start + 1
        plant_end = (  # final year plant is in production
            plant_start + plant_lifetime - 1
        ...
        ### Feed parameters
        # Total feedstock available for recycling each year
        Available_feed = get_hdd_common_params["Available_feed"]
        # collection rate for how much of the available feed is processed by the plant each year (fraction)
        CR = get_hdd_common_params["CR"]
        # list of tracked components
        Tracked_comps = get_hdd_common_params["Tracked_comps"]
        # mass of tracked component per EOL Product (metric tonnes of component / 1000 EOL product)
        Prod_comp_mass = get_hdd_common_params["Prod_comp_mass"]
        # calculate feed entering parameter based on yearly available feedstock and collection rate
        Feed_entering = copy.deepcopy(Available_feed)
        for key in Feed_entering:
            Feed_entering[key] = Available_feed[key] * CR
        ...
        ###################################################################################################
        ### Superstructure formulation parameters
        numStages = get_hdd_common_params["numStages"]  # no. of total stages
        # number of options in each stage
        Options_in_stage = get_hdd_common_params["Options_in_stage"]
        # set of options k' in stage j+1 connected to option k in stage j
        Option_outlets = get_hdd_common_params["Option_outlets"]
        # dictionary of tracked component retention efficiency for each option
        Option_Eff = get_hdd_common_params["Option_Eff"]
        maxOptions = max(Options_in_stage.values())  # max options in any of the stages
        # make a list of the options in the final stage
        final_opt_list = [(numStages, j) for j in RangeSet(Options_in_stage[numStages])]
        ###################################################################################################
        ...
        ### Operating Parameters
        # profit (k$) per metric tonnes of product in terms of tracked components
        Profit = get_hdd_common_params["Profit"]
        # conversion of kg REE/Fe to kg REO/Fe2O3
        REE_to_REO_Conversion = get_hdd_common_params["REE_to_REO_Conversion"]
        # For all options excluding the disassembly stage, the OPEX costs are linearly related to the flow entering it (metric tonnes).
        # OPEX = a*F_in + b*y
        N_OC_var = get_hdd_common_params["N_OC_var"]
        # number of workers, and type, needed by option (for disassembly stage, its operators per unit)
        num_workers = get_hdd_common_params["num_workers"]
        labor_rate = get_hdd_common_params[
            "labor_rate"
        ]  # yearly wage per type of labor
        # yearly operating costs per unit (k$/unit/yr)
        YCU = get_hdd_common_params["YCU"]
        # cost per disassembly stage unit for each disassembly option (k$/unit)
        CU = get_hdd_common_params["CU"]
        # disassembly rate (thousands of EOL HDDs/yr/unit)
        Dis_Rate = get_hdd_common_params["Dis_Rate"]
        # calculate max disassembly units possible for each option
        max_dis_by_option = copy.deepcopy(Dis_Rate)
        for key in max_dis_by_option.keys():
            max_dis_by_option[key] = math.ceil(maxFeedEntering / Dis_Rate[key])
        ...
        ###################################################################################################
        ### Costing Parameters
        LF = get_hdd_common_params["LF"]  # Lang Factor
        TOC_factor = get_hdd_common_params["TOC_factor"]  # Overnight costs factor
        ATWACC = get_hdd_common_params["ATWACC"]  # discount rate
        i_OC_esc = get_hdd_common_params["i_OC_esc"]  # opex, revenue escalation rate
        i_CAP_esc = get_hdd_common_params["i_CAP_esc"]  # capex escalation rate
        f_exp = get_hdd_common_params["f_exp"]  # capital expenditure schedule
        # Define Python Dictionary with discretized cost by flows for each option.
        Discretized_CAPEX = get_hdd_common_params["Discretized_CAPEX"]
        ###################################################################################################
        ###################################################################################################
        ### Other Parameters needed for tests
        # make a list of all options
...
class TestCOR(object):
    @pytest.fixture(scope="class")
    def COR_model(self, get_hdd_common_params):
        common_params = get_hdd_common_params
        m = build_model(
            ###################################################################################################
            ### Plant Lifetime Parameters
    ...
    def test_build(self, COR_model, get_hdd_common_params):
        # start of plant production
        prod_start = get_hdd_common_params["plant_start"] + 1
        # final year plant is in production
        plant_end = (
    ...
    @pytest.mark.skipif(not solver_available, reason="Gurobi solver not available")
    @pytest.mark.component
    def test_solution(self, COR_model, get_hdd_common_params):
        ##### Define Parameters for tests
        ###################################################################################################
        ### Plant Lifetime Parameters
        plant_start = get_hdd_common_params["plant_start"]  # start of plant production
...
class TestEnvironmentalImpacts(object):
    @pytest.fixture(scope="class")
    def EI_model(self, get_hdd_common_params):
        common_params = get_hdd_common_params
        m = build_model(
            ###################################################################################################
            ### Plant Lifetime Parameters
    ...
    def test_build(self, EI_model, get_hdd_common_params):
        # start of plant production
        prod_start = get_hdd_common_params["plant_start"] + 1
        # final year plant is in production
        plant_end = (
        ...
        operational_range = RangeSet(prod_start, plant_end)
        # no. of total stages
        numStages = get_hdd_common_params["numStages"]
        # number of options in each stage
        Options_in_stage = get_hdd_common_params["Options_in_stage"]
    ...
    @pytest.mark.skipif(not solver_available, reason="Gurobi solver not available")
    @pytest.mark.component
    def test_solution(self, EI_model, get_hdd_common_params):
        ##### Define Parameters for tests
        ###################################################################################################
        ### Plant Lifetime Parameters
        plant_start = get_hdd_common_params["plant_start"]  # start of plant production
        ...
        ###################################################################################################
        ### Consideration of environmental impacts parameters
        environ_impacts = get_hdd_common_params["environ_impacts"]
        ###################################################################################################
        ###################################################################################################
        ### Other Parameters needed for tests
        # make a list of all options
...
class TestByprodVal(object):
    @pytest.fixture(scope="class")
    def BV_model(self, get_hdd_common_params):
        common_params = get_hdd_common_params
        m = build_model(
            ###################################################################################################
            ### Plant Lifetime Parameters
    ...
    def test_build(self, BV_model, get_hdd_common_params):
        # start of plant production
        prod_start = get_hdd_common_params["plant_start"] + 1
        # final year plant is in production
        plant_end = (
    ...
    @pytest.mark.skipif(not solver_available, reason="Gurobi solver not available")
    @pytest.mark.component
    def test_solution(self, BV_model, get_hdd_common_params):
        ##### Define Parameters for tests
        ###################################################################################################
        ### Plant Lifetime Parameters
        plant_start = get_hdd_common_params["plant_start"]  # start of plant production
        ...
        CR = get_hdd_common_params["CR"]
        # list of tracked components
        Tracked_comps = get_hdd_common_params["Tracked_comps"]  # tracked components
        # mass of tracked component per EOL Product (metric tonnes of component / 1000 EOL product)
        Prod_comp_mass = get_hdd_common_params["Prod_comp_mass"]
        # calculate feed entering parameter based on yearly available feedstock and collection rate
        Feed_entering = copy.deepcopy(Available_feed)
        ...
        ### Byproduct valorization
        # list of byproducts
        byprods = get_hdd_common_params["byprods"]
        # dictionary of values for each byproduct (k$/metric tonnes). Negative value indicates it cost money to dispose of the byproduct
        byprod_vals = get_hdd_common_params["byprod_vals"]
        # dictionary keeping track of which tracked component produces which byproduct
        tracked_comp_for_byprod = get_hdd_common_params["tracked_comp_for_byprod"]
        # dictionary tracking which options produce a given byproduct
        byprod_options = get_hdd_common_params["byprod_options"]
        # dictionary tracking byproduct recovery efficiency for each option (in terms of tracked component)
        byprod_options_eff = get_hdd_common_params["byprod_options_eff"]
        # Conversion factors of tracked component to byproduct (metric tonnes byproduct / metric tonnes tracked component)
        TC_to_byproduct = get_hdd_common_params["TC_to_byproduct"]
        ###################################################################################################
        ###################################################################################################
        ### Other Parameters needed for tests
        # make a list of all options
...
# Check if function exits if an incorrect objective function is chosen.
class TestNoObjectiveFunction(object):
    def test_no_objective_chosen(self, get_hdd_common_params):
        common_params = get_hdd_common_params
