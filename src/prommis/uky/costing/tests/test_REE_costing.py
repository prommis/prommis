...
def test_REE_costing(m):
    ...
    reagent_costs = (  # all USD/year
        ...
        ) * pyunits.kg / pyunits.a
    
    m.fs.reagents = pyo.Var(m.fs.time, initialize=reagent_costs/(m.fs.annual_operating_hours), units=pyunits.kg/pyunits.hr)
...
def test_results(m):
    ...
    assert m.fs.costing.total_variable_OM_cost[0].value == pytest.approx(532.90, rel=1e-4)
    assert pyo.value(m.fs.costing.land_cost) == pytest.approx(1.2247, rel=1e-4)  # Expression, not Var
...
def test_costing_bounding(m):
    ...
    for key in m.fs.costing.costing_lower_bound.keys():
        assert (m.fs.costing.costing_lower_bound[key].value ==
                pytest.approx(expected_costing_lower_bound[key], rel=1e-4)
                )

    for key in m.fs.costing.costing_upper_bound.keys():
        assert (m.fs.costing.costing_upper_bound[key].value ==
                pytest.approx(expected_costing_upper_bound[key], rel=1e-4)
                )
