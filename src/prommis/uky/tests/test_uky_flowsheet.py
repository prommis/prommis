...
class TestUKyFlowsheet:
    ...
    def test_solve_flowsheet(self, scaled_model):
        ...
        assert scaled_model.fs.precipitator.cv_aqueous.properties_out[0].conc_mass_comp[
            "Fe"
        ].value == pytest.approx(4.9530, 1e-4)
        assert scaled_model.fs.precipitator.cv_aqueous.properties_out[0].conc_mass_comp[
            "Gd"
        ].value == pytest.approx(120.003, 1e-4)
        assert scaled_model.fs.precipitator.cv_aqueous.properties_out[0].conc_mass_comp[
            "La"
        ].value == pytest.approx(1978.54751, 1e-4)
        assert scaled_model.fs.precipitator.cv_aqueous.properties_out[0].conc_mass_comp[
            "Nd"
        ].value == pytest.approx(953.0926, 1e-4)
        assert scaled_model.fs.precipitator.cv_aqueous.properties_out[0].conc_mass_comp[
            "Pr"
        ].value == pytest.approx(261.256, 1e-4)
        assert scaled_model.fs.precipitator.cv_aqueous.properties_out[0].conc_mass_comp[
            "Sc"
        ].value == pytest.approx(9.20631, 1e-4)
        assert scaled_model.fs.precipitator.cv_aqueous.properties_out[0].conc_mass_comp[
            "Sm"
        ].value == pytest.approx(64.4434, 1e-4)
        assert scaled_model.fs.precipitator.cv_aqueous.properties_out[0].conc_mass_comp[
            "Y"
        ].value == pytest.approx(182.86048, 1e-4)

        assert scaled_model.fs.precipitator.precipitate_outlet.temperature[
            0
        ].value == pytest.approx(0.034815, 1e-4)
        assert scaled_model.fs.precipitator.precipitate_outlet.flow_mol_comp[
            0, "Al2(C2O4)3(s)"
        ].value == pytest.approx(5.838925e-4, 1e-4)
        assert scaled_model.fs.precipitator.precipitate_outlet.flow_mol_comp[
            0, "Ce2(C2O4)3(s)"
        ].value == pytest.approx(2.29770, 1e-4)
        assert scaled_model.fs.precipitator.precipitate_outlet.flow_mol_comp[
            0, "Dy2(C2O4)3(s)"
        ].value == pytest.approx(5.7957229e-2, 1e-4)
        assert scaled_model.fs.precipitator.precipitate_outlet.flow_mol_comp[
            0, "Fe2(C2O4)3(s)"
        ].value == pytest.approx(9.981925e-5, 1e-4)
        assert scaled_model.fs.precipitator.precipitate_outlet.flow_mol_comp[
            0, "Gd2(C2O4)3(s)"
        ].value == pytest.approx(0.2520741, 1e-4)
        assert scaled_model.fs.precipitator.precipitate_outlet.flow_mol_comp[
            0, "La2(C2O4)3(s)"
        ].value == pytest.approx(0.680896, 1e-4)
        assert scaled_model.fs.precipitator.precipitate_outlet.flow_mol_comp[
            0, "Nd2(C2O4)3(s)"
        ].value == pytest.approx(1.31427, 1e-4)
        assert scaled_model.fs.precipitator.precipitate_outlet.flow_mol_comp[
            0, "Pr2(C2O4)3(s)"
        ].value == pytest.approx(0.2958, 1e-4)
        assert scaled_model.fs.precipitator.precipitate_outlet.flow_mol_comp[
            0, "Sc2(C2O4)3(s)"
        ].value == pytest.approx(4.260289e-3, 1e-4)
        assert scaled_model.fs.precipitator.precipitate_outlet.flow_mol_comp[
            0, "Sm2(C2O4)3(s)"
        ].value == pytest.approx(0.133178, 1e-4)
        assert scaled_model.fs.precipitator.precipitate_outlet.flow_mol_comp[
            0, "Y2(C2O4)3(s)"
        ].value == pytest.approx(0.26984, 1e-4)
        ...
        assert scaled_model.fs.rougher_org_make_up.outlet.flow_vol[
            0
        ].value == pytest.approx(6.201, 1e-4)
        ...
        assert scaled_model.fs.acid_feed1.outlet.flow_vol[0].value == pytest.approx(
            0.09, 1e-4
        )
        assert scaled_model.fs.acid_feed2.outlet.flow_vol[0].value == pytest.approx(
            0.09, 1e-4
        )
        assert scaled_model.fs.acid_feed3.outlet.flow_vol[0].value == pytest.approx(
            9, 1e-4
        )
        ...
        assert scaled_model.fs.scrub_sep.inlet.flow_vol[0].value == pytest.approx(
            0.09, 1e-4
        )
        assert scaled_model.fs.rougher_mixer.outlet.flow_vol[0].value == pytest.approx(
            62.01, 1e-4
        )
        assert scaled_model.fs.sc_circuit_purge.inlet.flow_vol[
            0
        ].value == pytest.approx(6.201, 1e-4)
        ...
        assert scaled_model.fs.cleaner_org_make_up.outlet.flow_vol[
            0
        ].value == pytest.approx(6.201, 1e-4)
        ...
        assert scaled_model.fs.sl_sep2.recovered_liquid_outlet.flow_vol[
            0
        ].value == pytest.approx(0.063, 1e-4)
        assert scaled_model.fs.precip_sep.inlet.flow_vol[0].value == pytest.approx(
            0.063, 1e-4
        )
        assert scaled_model.fs.precip_sx_mixer.outlet.flow_vol[
            0
        ].value == pytest.approx(5.76, 1e-4)
    ...
    def test_costing(self, scaled_model):
        ...
        assert value(m.fs.costing.total_plant_cost) == pytest.approx(15.5759, rel=1e-4)
        assert value(m.fs.costing.total_BEC) == pytest.approx(5.244, rel=1e-4)
