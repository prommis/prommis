...
def test_solution(system_frame):
    ...
    assert value(model.fs.leach.solid_outlet.mass_frac_comp[
        0, "Al2O3"
    ...
    assert value(model.fs.leach.solid_outlet.mass_frac_comp[0, "Y2O3"]) == pytest.approx(
        2.9121365e-5, 1e-4
    )
    assert value(model.fs.leach.solid_outlet.mass_frac_comp[
        0, "inerts"
    ...
    assert value(model.fs.leach.liquid_outlet.conc_mass_comp[0, "H2O"]) == pytest.approx(
        1000000.0, 1e-4
    )
    assert value(model.fs.leach.liquid_outlet.conc_mass_comp[
        0, "HSO4"
    ]) == pytest.approx(694.61357, 1e-4)
    assert value(model.fs.leach.liquid_outlet.conc_mass_comp[0, "SO4"]) == pytest.approx(
        3961.7867, 1e-4
    )
    ...
    assert value(model.fs.solex_rougher_strip.mscontactor.organic_outlet.flow_vol[
        0
    ]) == pytest.approx(62.01, 1e-4)
    assert value(model.fs.solex_rougher_strip.mscontactor.organic_outlet.conc_mass_comp[
        0, "Al"
    ...
    assert value(model.fs.solex_rougher_strip.mscontactor.aqueous_outlet.flow_vol[
        0
    ]) == pytest.approx(0.09, 1e-4)
    assert value(model.fs.solex_rougher_strip.mscontactor.aqueous_outlet.conc_mass_comp[
        0, "H2O"
    ...
    assert value(model.fs.solex_cleaner_strip.mscontactor.organic_outlet.flow_vol[
        0
    ]) == pytest.approx(62.010, 1e-4)
    assert value(model.fs.solex_cleaner_strip.mscontactor.organic_outlet.conc_mass_comp[
        0, "Al"
    ...
    assert value(model.fs.solex_cleaner_strip.mscontactor.aqueous_outlet.flow_vol[
        0
    ]) == pytest.approx(9, 1e-4)
    assert value(model.fs.solex_cleaner_strip.mscontactor.aqueous_outlet.conc_mass_comp[
        0, "Al"
    ...
    assert value(model.fs.precipitator.cv_aqueous.properties_out[
        0
    ].flow_vol) == pytest.approx(9, 1e-4)
    assert value(model.fs.precipitator.cv_aqueous.properties_out[0].conc_mass_comp[
        "Al"
    ...
    assert value(model.fs.precipitator.precipitate_outlet.temperature[
        0
    ]) == pytest.approx(348.15, 1e-4)
    assert value(model.fs.precipitator.precipitate_outlet.flow_mol_comp[
        0, "Al2(C2O4)3(s)"
    ...
    assert value(model.fs.solex_rougher_load.mscontactor.aqueous_outlet.flow_vol[
        0
    ]) == pytest.approx(445.8968, 1e-4)
    assert value(model.fs.solex_rougher_scrub.mscontactor.aqueous_outlet.flow_vol[
        0
    ]) == pytest.approx(0.09, 1e-4)
    assert value(model.fs.solex_rougher_strip.mscontactor.aqueous_outlet.flow_vol[
        0
    ...
    assert value(model.fs.rougher_mixer.outlet.flow_vol[0]) == pytest.approx(62.01, 1e-4)
    assert value(model.fs.sc_circuit_purge.inlet.flow_vol[0]) == pytest.approx(
        6.201, 1e-4
    )
    assert value(model.fs.solex_cleaner_load.mscontactor.aqueous_outlet.flow_vol[
        0
    ...
    assert value(model.fs.cleaner_mixer.outlet.flow_vol[0]) == pytest.approx(62.01, 1e-4)
...
def test_costing_solution(system_frame):
    ...
    assert value(model.fs.costing.total_fixed_OM_cost) == pytest.approx(6.8105, rel=1e-4)
